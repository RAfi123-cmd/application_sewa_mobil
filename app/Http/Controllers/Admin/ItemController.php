...
class ItemController extends Controller
{
            ...
            return DataTables::of($query)
                ...
                ->addColumn('action', function($item) {
                    ...
                    <form class="block w-full" onsubmit="return confirm(\'Apakah anda yakin?\');" -block" action="' . route('admin.items.destroy', $item->id) . '" method="POST">
                        <button class="w-full px-2 py-1 text-xs text-white transition  duration-500 bg-red-500 border border-red-500 select-none ease hover:bg-red-600 focus:outline-none focus:shadow-outline">
                            Hapus
                        </button>
    ...
    public function update(ItemRequest $request, Item $item)
    {
        ...
        if ($request->hasFile('photos')) {
            ...
            // If photos is empty, then us old photos
    ...
    public function destroy(Item $item)
    {
        ...
        return redirect()->route('admin.items.index')->with('success', 'Item berhasil dihapuskan');
    }
}
